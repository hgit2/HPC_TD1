...
map<char, int> Freqchar2(char** mat, int size1, int size2){
	// crée un objet map, freqc, contenant la liste des lettres de l'alphabet, associés à leur fréquence d'apparition dans la matrice mat
	
	std::map<char, int> freqc; 
	for(int i=97; i<=122; ++i){
		freqc.insert( std::pair<char,int> (char(i),0) ); // initialisation (remplissage de l'objet map avec les lettres de l'alphabet
	}
...
int coeur=atoi(argv[3]); // donne le nombre de coeurs
...
map<char, int> freqmat1 = Freqchar2(mat1, size1, size2); // Calcul du nombre d’occurences de chaque lettres présent dans mat1
int after=(clock()*1000)/CLOCKS_PER_SEC;
int diff=after - before;
cout << "temps d'execution du comptage de lettres d'une matrice " << diff << endl;

// Affichage du nombre d’occurences de chaque lettre présentes dans la matrice mat1
cout << "liste:"<<endl;
